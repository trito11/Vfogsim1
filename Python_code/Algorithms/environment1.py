...
from UserMatrix_Func_rl_new import UserMatrix_Func
...
servicediff = 0
traffic = preprocess("vehicles.csv", source)
...
UserMatrix = UserMatrix_Func(USERCOUNT, Simulation_Duration, traffic,  X_loc, Y_loc, sinr0, sinr1, sinr2, sinr3, sinr4, sinr5, sinr6, sinr7, sinr8, 19)
...
for n in range(Simulation_Duration):
    port = UserMatrix[:, X_loc, n] > 0
    dummy = Ozgur_User_driven_event(np.sum(port, 0), 3, 1, 1, EventRate, UserMatrix[:, X_loc, n] > 0)
...
for i in range(events.shape[0]):
    ...
    app_type = events[i, 2]
    demand = events[i, 3]
    UserMatrix[user, App_style, time_slot] = app_type
    UserMatrix[user, Deadline, time_slot] = np.random.randint(DEADLINE[0],DEADLINE[1])*100/100
    UserMatrix[user, GFLOPS, time_slot] = demand
...
class BusEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.observation[0] = self.task[GFLOPS] #REQUIRED_GPU_FLOPS
        self.observation[1] = self.task[s_in] #s_in
        self.observation[2] = self.task[s_out] #s_out
        self.observation[3] = self.task[Deadline] #deadline
    ...
    #Action là số kiểu int ko phải [] hoặc tensor
        ...
        if action > 0:
            ...
            Rate_trans_res_data=B_pow *np.log2(1 + self.task[action+9] / GAMMA)
