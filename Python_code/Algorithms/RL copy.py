...
class DQNAgent:
    ...
    def normalize(self, state):
        array = torch.zeros(1,25)
        array[0][0] = (state[0][0] - REQUIRED_GPU_FLOPS[0]) / (REQUIRED_GPU_FLOPS[1] - REQUIRED_GPU_FLOPS[0])
        array[0][1] = (state[0][1] - MIN_S_IN/100) / (MAX_S_IN - MIN_S_IN)*100
        array[0][2] = (state[0][2] - MIN_S_OUT/100) / (MAX_S_OUT - MIN_S_OUT)*100
        array[0][3] = (state[0][3] - DEADLINE[0]) / (DEADLINE[1] - DEADLINE[0])
        array[0][4]= state[0][3]/30/100

        for i in range(NUM_VEHICLE):
            array[0][i * 2 + 5] = state[0][i * 2 + 4] / 10
            array[0][i * 2 + 6] = state[0][i * 2 + 5] / 400
        return array
            ...
            if sample > self.eps_threshold:
                with torch.no_grad():
                    ...
                    for i in range(NUM_ACTION-1):
                        if state[0][i*2+5]<100:
                            m.append(i)
                    if len(m)!=0:
                        indices_of_interest = torch.tensor(m)
                        a=self.policy_net(state)[0][indices_of_interest]
                        a=torch.tensor([[a]])
                ...
                if a==0:
                    ...
                    b=m[random.randint(0,a-1)]
                return torch.tensor([[b]], device=device, dtype=torch.long)    
